...
dependencies needed for running iupred (`[tool.poetry.dependencies]` with the
...
developing iupred using test-driven development. Please make sure that you
...
If you are brand new to iupred or open-source development, we recommend
...
iupred is hosted on GitHub, and to contribute, you will need to sign up for a
...
### Create a fork of iupred

You will need your own fork of iupredin order to eventually open a Pull
Request. Go to the iupred project page and hit the Fork button. Please
...
```bash
git clone https://github.com/your-user-name/iupred.git
cd iupred
git remote add upstream https://github.com/iupred/iupred.git
git fetch upstream
```

This creates the directory `iupred` and connects your repository to the
upstream (main project) *iupred* repository. They have the same name, but
your local repository and fork are separate from the upstream repository.

### Creating a feature branch

Your local `main` branch should always reflect the current state of iupred
repository. First ensure it's up-to-date with the main iupred repository.
...
to create a branch called `my-new-feature-for-iupred`

```bash
git checkout -b my-new-feature-for-iupred
```

This changes your working branch from `main` to the
`my-new-feature-for-iupred` branch. Keep any changes in this branch specific
to one bug or feature so it is clear what the branch brings to *iupred*. You
...
When making changes, follow these iupred-specific guidelines:
...
Now your code is on GitHub, but it is not yet a part of the iupred project.
...
1. Write a descriptive title that includes prefixes. iupred uses a convention for title prefixes, most commonly, `feat:` for features, `fix:` for bug fixes, and `refactor:` for refactoring.
...
branch with changes in the iupred `main` branch, run:

```shell

    git checkout my-new-feature-for-iupred
    git fetch upstream
...
```shell
    git push origin my-new-feature-for-iupred
```
